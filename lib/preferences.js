...
pref.get(null, function(storage) {
	getPref = {
		// Hidden Preferences
		version: storage["version"],
		style: (!storage["style"] ? style : storage["style"]),
		updatechecktime: Date.now() - 36e5 * (5 - 1 / 12),
		videoFormat: videoFormat,
		localisedStrings: localisedStrings,
		// Check if it's the first time extension is running
		firstrun: 					(!storage["firstrun"]				? null		: storage["firstrun"]),
		// Normal preferences
			actions:				(!storage["actions"]				? {}		: storage["actions"]),
			addtoolbarbutton:		(!storage["addtoolbarbutton"]		? false		: storage["addtoolbarbutton"]),
			addbuttonpopup:			(!storage["addbuttonpopup"]			? true		: storage["addbuttonpopup"]),
			allowautoupdate:		(!storage["allowautoupdate"]		? false		: storage["allowautoupdate"]),
			buttonpopupalways:		(!storage["buttonpopupalways"]		? true		: storage["buttonpopupalways"]),
			cleanwatch: {
				allbuttons:			(!storage["allbuttons"]				? false		: storage["allbuttons"]),
					addtobutton:	(!storage["addtobutton"]			? false		: storage["addtobutton"]),
					likebutton:		(!storage["likebutton"]				? false		: storage["likebutton"]),
					flagbutton:		(!storage["flagbutton"]				? false		: storage["flagbutton"]),
					sharebutton:	(!storage["sharebutton"]			? false		: storage["sharebutton"]),
					statsbutton:	(!storage["statsbutton"]			? false		: storage["statsbutton"]),
					transcript:		(!storage["transcript"]				? false		: storage["transcript"]),
				allcomments:		(!storage["allcomments"]			? false		: storage["allcomments"]),
					comments:		(!storage["comments"]				? false		: storage["comments"]),
					responses:		(!storage["responses"]				? false		: storage["responses"]),
					toprated:		(!storage["toprated"]				? false		: storage["toprated"]),
					uploader:		(!storage["uploader"]				? false		: storage["uploader"]),
				allsidebar:			(!storage["allsidebar"]				? false		: storage["allsidebar"]),
					featured:		(!storage["featured"]				? false		: storage["featured"]),
					suggestions:	(!storage["suggestions"]			? false		: storage["suggestions"]),
				brand:				(!storage["brand"]					? false		: storage["brand"]),
				description:		(!storage["description"]			? false		: storage["description"]),
				footer:				(!storage["footer"]					? false		: storage["footer"]),
				header:				(!storage["header"]					? false		: storage["header"]),
				headuser:			(!storage["headuser"]				? false		: storage["headuser"]),
				langalert:			(!storage["langalert"]				? false		: storage["langalert"]),
				subscribe:			(!storage["subscribe"]				? false		: storage["subscribe"]),
				videotitle:			(!storage["videotitle"]				? false		: storage["videotitle"]),
				viewcount:			(!storage["viewcount"]				? false		: storage["viewcount"]),
			},
			customcolor:{
				badgeBackground: "hsl(0, 0%, 95%)",
				badgeText: "hsl(0, 0%, 33%)",
			...
			customcolorsinpopup:	(!storage["customcolorsinpopup"]	? false		: storage["customcolorsinpopup"]),
			customlogo:				(!storage["customlogo"]				? false		: storage["customlogo"]),
			customstyle:			(!storage["customstyle"]			? ""		: storage["customstyle"]),
			disableAutoPlay:		(!storage["disableAutoPlay"]		? false		: storage["disableAutoPlay"]),
			disableautobuffering:	(!storage["disableautobuffering"]	? false		: storage["disableautobuffering"]),
			disableflashshortcuts:	(!storage["disableflashshortcuts"]	? /Linux|BSD|Mac/.test(window.navigator.platform) : storage["disableflashshortcuts"]),
			enablecustomcolors:		(!storage["enablecustomcolors"]		? false		: storage["enablecustomcolors"]),
			enablecustomstyle:		(!storage["enablecustomstyle"]		? false		: storage["enablecustomstyle"]),
			enabledownload:			(!storage["enabledownload"]			? false		: storage["enabledownload"]),
			exposeplayershortcuts:	(!storage["exposeplayershortcuts"]	? false		: storage["exposeplayershortcuts"]),
			enablepopout:			(!storage["enablepopout"]			? false		: storage["enablepopout"]),
			enableshortcutkeys:		(!storage["enableshortcutkeys"]		? false		: storage["enableshortcutkeys"]),
			enablewideplayer:		(!storage["enablewideplayer"]		? false		: storage["enablewideplayer"]),
			extensionwriteplayer:	(!storage["extensionwriteplayer"]	? false		: storage["extensionwriteplayer"]),
			extrabuttons:			(!storage["extrabuttons"]			? false		: storage["extrabuttons"]),
			forceloop:				(!storage["forceloop"]				? false		: storage["forceloop"]),
			forceplayonfocus:		(!storage["forceplayonfocus"]		? false		: storage["forceplayonfocus"]),
			fullscreenstyle:		(!storage["fullscreenstyle"]		? false		: storage["fullscreenstyle"]),
			hideannotations:		(!storage["hideannotations"]		? false		: storage["hideannotations"]),
			hidecontrols:			(!storage["hidecontrols"]			? 3			: storage["hidecontrols"]),
			hidepageads:			(!storage["hidepageads"]			? true		: storage["hidepageads"]),
			hideplayerads:			(!storage["hideplayerads"]			? true		: storage["hideplayerads"]),
			howwide:				(!storage["howwide"]				? "auto"	: storage["howwide"]),
			locale:					(!storage["locale"]					? "en"		: storage["locale"]),
			loglevel:				(!storage["loglevel"]				? 1			: storage["loglevel"]),
			loop:					(!storage["loop"]					? false		: storage["loop"]),
			loopbutton:				(!storage["loopbutton"]				? false		: storage["loopbutton"]),
			lyrics:					(!storage["lyrics"]					? false		: storage["lyrics"]),
			lyricsenablealways:		(!storage["lyricsenablealways"]		? false		: storage["lyricsenablealways"]),
			lyricssearchlog:		(!storage["lyricssearchlog"]		? false		: storage["lyricssearchlog"]),
			onlyonfirstfocus:		(!storage["onlyonfirstfocus"]		? false		: storage["onlyonfirstfocus"]),
			overridehistory:		(!storage["overridehistory"]		? true		: storage["overridehistory"]),
			overridelocale:			(!storage["overridelocale"]			? false		: storage["overridelocale"]),
			playertheme:			(!storage["playertheme"]			? "dark"	: storage["playertheme"]),
			playonfocus:			(!storage["playonfocus"]			? false		: storage["playonfocus"]),
			popoutrealsize:			(!storage["popoutrealsize"]			? false		: storage["popoutrealsize"]),
			popupupdateinterval:	(!storage["popupupdateinterval"]	? 456		: storage["popupupdateinterval"]),
			prefbutton:				(!storage["prefbutton"]				? false		: storage["prefbutton"]),
			preventcrossplay:		(!storage["preventcrossplay"]		? false		: storage["preventcrossplay"]),
			progresscolor:			(!storage["progresscolor"]			? "red"		: storage["progresscolor"]),
			ratevideos:				(!storage["ratevideos"]				? false		: storage["ratevideos"]),
			replace:				(!storage["replace"]				? ""		: storage["replace"]),
			scrollonlyricsdisplay:	(!storage["scrollonlyricsdisplay"]	? true		: storage["scrollonlyricsdisplay"]),
			seektime:				(!storage["seektime"]				? 3			: storage["seektime"]),
			shortcut: {
				expandplayer:		(!storage["expandplayer"]			? ""		: storage["expandplayer"]),
				framestep:			(!storage["framestep"]				? ""		: storage["framestep"]),
				hideall:			(!storage["hideall"]				? ""		: storage["hideall"]),
				loopsection:		(!storage["loopsection"]			? ""		: storage["loopsection"]),
				marksectionend:		(!storage["marksectionend"]			? ""		: storage["marksectionend"]),
				marksectionstart:	(!storage["marksectionstart"]		? ""		: storage["marksectionstart"]),
				playpause:			(!storage["playpause"]				? ""		: storage["playpause"]),
				popplayer:			(!storage["popplayer"]				? ""		: storage["popplayer"]),
				seekback:			(!storage["seekback"]				? ""		: storage["seekback"]),
				seekforward:		(!storage["seekforward"]			? ""		: storage["seekforward"]),
				showlyrics:			(!storage["showlyrics"]				? ""		: storage["showlyrics"]),
				showhidden:			(!storage["showhidden"]				? ""		: storage["showhidden"]),
				togglecolors:		(!storage["togglecolors"]			? ""		: storage["togglecolors"]),
				togglecss:			(!storage["togglecss"]				? ""		: storage["togglecss"]),
			},
			thumbhoverdelay:		(!storage["thumbhoverdelay"]		? 3000		: storage["thumbhoverdelay"]),
			thumbpreview:			(!storage["thumbpreview"]			? false		: storage["thumbpreview"]),
			thumbpreviewtrigger:	(!storage["thumbpreviewtrigger"]	? "button"	: storage["thumbpreviewtrigger"]),
			unapprovedcheck:		(!storage["unapprovedcheck"]		? false		: storage["unapprovedcheck"]),
			updatecheck:			(!storage["updatecheck"]			? true		: storage["updatecheck"]),
			updateinterval:			(!storage["updateinterval"]			? 5			: storage["updateinterval"]),
			usefallbacklinks:		(!storage["usefallbacklinks"]		? false		: storage["usefallbacklinks"]),
			uselighticons:			(!storage["uselighticons"]			? false		: storage["uselighticons"]),
			videoquality:			(!storage["videoquality"]			? "default"	: storage["videoquality"])
		}
})
...
function comparePrefs() {
	var version = getPref["version"],
		defs = Object.union(defaultPreferences, hiddenPreferences)

	if (version && String.natcmp(version, extVersion) < 0) {
		exportPreferences()

		pref.set({"style": defaultPreferences.style})
		pref.set({"localisedStrings": defaultPreferences.localisedStrings})
		pref.set({"videoFormat": defaultPreferences.videoFormat})
	}
	...
	if (changedItems.length) {
		...
		changedItems.forEach(function (item) {
			...
			if (key[1]) {
				newPref = getPref[key[0]]
				newPref[key[1]] = defs[key[0]][key[1]]
			}

			pref.set({value: newPref})
		})
...
function loadDefaultPreferences() {
	for (var key in defaultPreferences) {
		...
		pref.set({key: defaultPreferences[key]})
	}
...
function loadRecommendedPreferences(all) {
	for (var key in recommendedPreferences) {
		if (!all) {
			if ("customstyle" == key && getPref[key] != "")
				continue
			else if ("actions" == key && getPref[key] != "{}")
				continue
		}

		if (Object.isObject(recommendedPreferences[key]))
			pref.set({key: Object.union(recommendedPreferences[key], getPref[key])})
		else
			pref.set({key: recommendedPreferences[key]})
	}
...
//    version: " + pref.getItem("version") + "\n\n"
	...
	if (bgProcess)
		...
		bgProcess.chrome.tabs.create(tab)
	else
...
function fixCustomColors() {
	var customcolor = getPref["customcolor"],
		changed = false,
		oldcolor = {
	...
	if (changed) {
		pref.set({"customcolor": customcolor})
...
function fixAutoPlay() {
	if (getPref["disableautoplay"] !== null) {
		pref.set({"preventcrossplay": getPref["disableautoplay"]})
		pref.remove("disableautoplay")
	}

	if (getPref["disableautoplayalways"] !== null) {
		pref.set({"disableAutoPlay": getPref["disableautoplayalways"]})
		pref.remove("disableautoplayalways")
	}

	if (getPref["channelautoplay"] !== null)
		pref.remove("channelautoplay")
}
