...
var bgProcess = chrome.extension.getBackgroundPage(),
	...
	extVersion = chrome.app.getDetails().version.replace(/\-.+/, ""),
	availableUpdate = ""

/*function getPref(key) {
	pref.get(key, function(a) {
		return a.key
	})
}*/
...
if (bgProcess)
	...
	var log = {
			...
			Info: function () {
				var loglevel = getPref["loglevel"]
				pref.set({"loglevel": 3})
				this.info.apply(this, arguments)
				pref.set({"loglevel": loglevel})
			},
			warn: function () {
				var mesg = reduce.apply(this, arguments).trim("\n")
				mesg = getCallerName() + ": " + mesg
			...
			error: function () {
				...
				if (getPref["loglevel"] > 0 || getPref["loglevel"] === null)
...
window.addEventListener("load", function () {
	Array.prototype.forEach.call(document.querySelectorAll("a[href^=\"mailto:\"]"), function (mail) {
		mail.addEventListener("click", function (event) {
			...
			bgProcess.chrome.tabs.create({ url: this.href })
		}, false)
	})
}, false)
