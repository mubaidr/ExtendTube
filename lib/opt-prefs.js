...
function fillPreferences() {
	...
	Array.prototype.forEach.call(control, function (element) {
		...
		if (group[1]) {
			...
			if (element.type == "text" || element.type == "number")
				element.value = getPref[element.id]
			else if (element.type == "checkbox")
				element.checked = getPref[element.id]
			else if (element.type == "select-one")
				Array.prototype.forEach.call(element, function (option) {
					if (option.value == getPref[element.id])
						option.selected = true
				})
		}
	})

	var customstyle = getPref["customstyle"]
	if (customstyle)
		document.getElementById("customstyle").value = customstyle

	// log.info('opt-prefs:',
	console.info('opt-prefs:',
...
function optionChanged(event) {
	if (this.type == "checkbox") {
		...
		if (group[1] && group[0] == "cleanwatch") {
			var cleanwatch = getPref["cleanwatch"],
			...
			pref.set({"cleanwatch": cleanwatch})
		}
		else {
			// log.info('opt-prefs:',
			console.info('opt-prefs:',
					 'An option is changed.',
					 'ID: ' + this.id + '.',
					 'From: ' + getPref[this.id] + '.',
					 'To: ' + this.checked + '.')

			var value = this.id
			pref.set({value: this.checked})
		}
	}
...
function importPreferences(event) {
	...
	for (var key in imported) {
		if (Object.isObject(imported[key]))
			pref.set({key: Object.union(getPref[key], imported[key], true)})
		else
			pref.set({key: imported[key]})
	}
...
function storageGetUsedSpace() {
	return Object.keys(pref).reduce(function (length, key) {
			if (key == "length")
				return length
			return length + key.length + getPref[key].length
		}, 0)
}
...
function storageGetAvailableSpace() {
	...
	while (testString.length) {
		...
		try {
			var value = "QuotaTest" + N
			pref.set({value: testString})
			testKeys.push(value)
